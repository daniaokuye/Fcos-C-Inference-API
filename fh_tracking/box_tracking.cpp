...
bool BoxTracker::near_boundary(Rect box1) {
    ...
    bool x1_boundary = float(box1.x) < std::min(50.0f, img_w * near_boundary_th);
    bool y1_boundary = float(box1.y) < std::min(50.0f, img_h * near_boundary_th);
    bool x2_boundary = float(img_w - x2) < std::min(50.0f, img_w * near_boundary_th);
    bool y2_boundary = float(img_h - y2) < std::min(50.0f, img_h * near_boundary_th);
...
float BoxTracker::cal_total_cost(Rect box1, tracker_info info) {
    ...
    float distance_cost = sqrt((center_x1 - center_x2) * (center_x1 - center_x2) +
                               (center_y1 - center_y2) * (center_y1 - center_y2));//[0,++)
    ...
    if (size_cost > 2 || ratio_cost > 1.5 || distance_cost > 1.5) {//1,1,1
        total_cost = max_cost;
        return total_cost;
    }

    total_cost = iou_cost * iou_cost_weight + size_cost * size_cost_weight + ratio_cost * ratio_cost_weight + \
            distance_cost * distance_cost_weight + mismatchTimes_cost * mismatchTimes_cost_weight +
                 boundary_cost * boundary_cost_weight;
    ...
    if (frames_num == 0) {
        ...
        for (int id = 0; id < tracking_infos_size; id++) {
            ...
            if (out_boundary_delete) {
                //如果跟踪器出边缘，快时间内删除跟踪器
                if (near_boundary(tracking_infos[id].rect) &&
            ...
            ////////////////////////offshift
            for (auto line :cut_line) {
                int left = tracking_infos[id].rect.x - line,
                        right = line - tracking_infos[id].rect.x + tracking_infos[id].rect.width;
                bool near_cut = (left < 2 && left >= 0) && (right >= 0 && right < 2);
                if (!is_time_out and near_cut and tracking_infos[id].mismatch_times > 2) {
                    tracking_infos[id].mismatch_times = tracking_infos[id].mismatch_times + 3;//(n+1)x > 30
                }
            }
            ////////////////////////
        }
...
void BoxTracker::del_ids(int *ids, int id_num) {
    ////////////////////////not now!
    for (int i = 0; i < id_num; i++) {
        auto id_ = ids[i];
        tracking_infos[id_].mismatch_times = max_mismatch_times;//has one more chance
    }
    ////////////////////////
}