...
public:
    BoxTracker(float iou_cost_weight_, float cost_th_, int max_mismatch_times_) {
        ...
        max_cost = iou_cost_weight + distance_cost_weight + size_cost_weight + ratio_cost_weight +
                   mismatchTimes_cost_weight + boundary_cost_weight;

    }

    ~BoxTracker() {}

    vector<int> tracking_Frame_Hungarian(vector <Rect>, int, int, vector<int>);

    void del_ids(int *ids, int id_num);
