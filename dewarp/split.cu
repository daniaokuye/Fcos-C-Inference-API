...
splitProcess::splitProcess(int w, int h, int c, int outW, int outH, int slots, int pad) :
    ...
    cudasafe(cudaMalloc((void **) &pixels_dev, newImageByteLength * slots),
             "New image allocation ", __FILE__, __LINE__);
    cudasafe(cudaEventCreate(&start), "start allocation ", __FILE__, __LINE__);
    cudasafe(cudaEventCreate(&stop), "stop allocation ", __FILE__, __LINE__);
    count = 0;
}
...
splitProcess::~splitProcess() {
    // Free memory
    ipt_pixels_dyn = NULL;
    cudasafe(cudaFree(pixels_dev), "free cuda output ", __FILE__, __LINE__);
    cudasafe(cudaEventDestroy(start), "destory start ", __FILE__, __LINE__);
    cudasafe(cudaEventDestroy(stop), "destory stop ", __FILE__, __LINE__);
}
...
//    printf("CUDA dst: [%p]", out_Image);
    cudasafe(cudaMemcpy(out_Image, ipt_pixels_dyn, imageByteLength, cudaMemcpyDeviceToHost),
...
void splitProcess::test_sv() {
    ...
    cudasafe(cudaMemcpy(dst.data, out_Pixels_dyn, sz,
...
//    int out_w = 1280, out_h = 640;
    ...
    cudasafe(cudaMemcpy(pixels_dyn, src.data, imageByteLength, cudaMemcpyHostToDevice),
...
//    cudasafe(cudaMemcpy(dst.data, newPixels_dyn + newImageByteLength / subbatch, newImageByteLength / subbatch,
//                        cudaMemcpyDeviceToHost), "from device to host", __FILE__, __LINE__);
    cudasafe(cudaMemcpy(dst.data, newPixels_dyn, newImageByteLength,
                        cudaMemcpyDeviceToHost), "from device to host", __FILE__, __LINE__);
